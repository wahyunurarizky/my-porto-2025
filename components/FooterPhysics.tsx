...
export function FooterPhysics({ boardTextureURLs = [], className }: FooterPhysicsProps) {
  ...
  const limitedBoardTextures = boardTextureURLs;
  ...
  useEffect(() => {
    ...
    const boards = limitedBoardTextures.map((texture) => {
      ...
      return Bodies.rectangle(x, y, isMobile ? 50 : 100, isMobile ? 50 : 100, {
        chamfer: { radius: 10 }, // Rounded corners for accurate collision
        angle: rotation,
        ...
        render: {
          sprite: {
            texture,
            xScale: isMobile ? 0.5 : 1, // Scale texture down
